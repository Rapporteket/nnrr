...
<<'Indikator 10 v2: andel tilbake i arbeid', results='asis', echo=FALSE>>=

aux <- Oppf_data[which(Oppf_data$regstatus==1 & Oppf_data$regstatus_post==1), ]

# table(aux$IsEmployed, useNA = 'ifany')
# table(aux$SickLeave, useNA = 'ifany')
# table(aux$S2_SickLeave, useNA = 'ifany')
#
# aux$S2_SickLeave[is.na(aux$S2_SickLeave)] <- aux$SickLeave[is.na(aux$S2_SickLeave)]

aux <- aux[!is.na(aux$S2_SickLeave) & !is.na(aux$S2_SickLeave_post), ]
aux <- aux[aux$IsEmployed == 1 & aux$S2_SickLeave == 1, ]
...
tmp <- aux[which(aux$S2_SickLeave_Percent_post < aux$S2_SickLeave_Percent), ]
@
