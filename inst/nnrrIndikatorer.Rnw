...
format_ut <- ".pdf"
figfolder <- "~/GIT/nnrr/inst/indikatorer_2021/"
...
nnrrFigIndikator(indikator, c('Andel tilbake i arbeid eller', 'med lavere andel sykemelding' ),
                 outfile=outfile, maal = 30, minstekrav = 25)
if (format_ut != "pdf") {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), format_ut)
  nnrrFigIndikator(indikator, c('Andel tilbake i arbeid eller', 'med lavere andel sykemelding' ), outfile=outfile, maal = 30, minstekrav = 25)
}
...
\caption{Andel tilbake i arbeid etter 6 mnd, ny versjon.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}tilbake_i_arbeid_ny_v2.pdf}
\caption{Andel tilbake i arbeid, eller med lavere grad av sykemelding etter 6 mnd.}
\end{figure}
...
# write.csv2(Indikatorer, paste0('~/data/nnrr/nnrr_ind_', Sys.Date(), '.csv'), row.names = F, fileEncoding = "UTF-8")

dg <- readr::read_csv2("~/data/nnrr/dg_nnrr_raa.csv", locale = locale(encoding = "Latin1"))
...
map_dg2 <- data.frame(enhet=c("Sandnessjøen", "St Olavs", "Harstad", "Tromsø",
                              ...
                              "Haukeland", "Drammen", "Levanger"),
                      avdresh = c(102169, 104293, 700735, 601032, 4211588, 109834,
                                  4212982, 114174, 102959, 103736, 105821))

dg_ny <- readr::read_csv2("~/data/nnrr/Dekningsgrad 2021 NNRR.csv", locale = locale(encoding = "Latin1"))
...
# write.csv2(dg, paste0('~/data/nnrr/dg_nnrr_', Sys.Date(), '.csv'), row.names = F, fileEncoding = "UTF-8")
@
