...
format_ut <- ".pdf"
figfolder <- "~/nnrr/inst/indikatorer_2021/"
...
<<'Nokkeltall NNRR', results='asis', echo=FALSE, message=FALSE>>=
...
Indikatorer <- Indikatorer[!(Indikatorer$ind_id == "nnrr_jobb" & Indikatorer$year > 2020 ), ] # Fjerner arbeidsind for nyere registrernger
                                                                                              # Ny variant kommer
# write.csv2(Indikatorer, paste0('~/.ssh/data/nnrr/nnrr_ind_', Sys.Date(), '.csv'), row.names = F, fileEncoding = "UTF-8")

dg <- readr::read_csv2("~/.ssh/data/nnrr/dg_nnrr_raa.csv",
...
dg_ny <- readr::read_csv2("~/.ssh/data/nnrr/Dekningsgrad 2021 NNRR.csv",
                          locale = readr::locale(encoding = "Latin1"))
...
# write.csv2(Indikatorer, paste0('~/.ssh/data/nnrr/indikatorer_nnrr_', Sys.Date(), '.csv'), row.names = F, fileEncoding = "UTF-8")
@
