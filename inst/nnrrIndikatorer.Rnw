...
datasti <- "C:/Users/kth200/regdata/nnrr/behandlingskvalitet/"
RegData <- nnrr::nnrrHentRegData()

RegDataAll <- RegData
RegData <- RegData[which(RegData$Aar >= 2022 & RegData$Aar <= rap_aar), ]
Oppf_data <- RegDataAll[which(RegDataAll$aar_oppfolg>= 2022 & RegDataAll$aar_oppfolg <= rap_aar), ]
...
format_ut <- ".svg"
figfolder <- paste0(datasti, "indikatorer_2025/")
if (!dir.exists(figfolder)) {
  dir.create(figfolder)
}

figstr <- 0.61
# knitr::knit2pdf("nnrrIndikatorer.Rnw")
@
...
skjema1b$tverrfaglig_behandlet[
  skjema1b$Treatment_GroupInterdisciplinary2018 != 0 | skjema1b$Treatment_GroupInterdisciplinary != 0] <- 1
skjema1b$tverrfaglig_behandlet[
  skjema1b$Treatment_InvidualInterdisciplinary != 0] <- 1

tabell1 <- nnrrIndikatorTabell(skjema1b |> dplyr::filter(Aar > rap_aar-4),
                               indikator = 'tverrfaglig_behandlet')
print(xtable::xtable(tabell1,
...
tabell1 <- nnrrIndikatorTabell(aux |> dplyr::filter(Aar > rap_aar-4),
                               indikator = 'odidiff_klin_viktig')
print(xtable::xtable(tabell1,
                     digits=c(0,0,0,0,0,1),
                     caption=paste0('Andel med klinisk viktig endring i ODI
                     (minimum 10 poeng reduksjon). I hele perioden er det ',
                     sum(aux$OdiScore[which(aux$Aar > rap_aar-4)] < 10), '
                     pasienter som har prescore $<$ 10')),
      include.rownames=FALSE)
#########################################################################################
@
...
tabell1 <- nnrrIndikatorTabell(aux |> dplyr::filter(Aar > rap_aar-4),
                               indikator = 'ndidiff_klin_viktig')
print(xtable::xtable(tabell1,
                     digits=c(0,0,0,0,0,1),
                     caption=paste0('Andel med klinisk viktig endring i NDI
                     (minimum 10 poeng reduksjon). I hele perioden er det ',
                     sum(aux$OdiScore[which(aux$Aar > rap_aar-4)] < 10), '
                     pasienter som har prescore $<$ 10')),
      include.rownames=FALSE)
...
tabell1 <- nnrrIndikatorTabell(aux |> dplyr::filter(Aar > rap_aar-4),
                               indikator = 'minimal_funksjonsnedsettelse')
print(xtable::xtable(tabell1,
                     digits=c(0,0,0,0,0,1),
                     caption='Andel med minimal funksjonsnedsettelse etter 6
                     mnd. (ODI score oppfølging $\\leq$ 23)'),
      include.rownames=FALSE)
...
print(xtable::xtable(tabell1,
                     digits=c(0,0,0,0,0,1),
                     caption='Andel med minimal funksjonsnedsettelse etter
                     6 mnd. (NDI score oppfølging $\\leq$ 20)'),
      include.rownames=FALSE)
...
# aux$pstEndringSmerteHvile <-
#   (aux$PainExperiencesNoActivity - aux$PainExperiencesNoActivity_post)/
#   aux$PainExperiencesNoActivity*100

aux$Indikator <- 0
# aux$Indikator[aux$pstEndringSmerteHvile >= 30 ] <- 1
aux$Indikator[(aux$PainExperiencesNoActivity -
                 aux$PainExperiencesNoActivity_post) >= 2] <- 1
...
print(xtable::xtable(
  ...
  digits=c(0,0,0,0,0,1),
  caption=paste0(
    'Andel med 2 poeng eller mer reduksjon i smerte i hvile. Individer med
    smertescore = 0 ved inklusjon er fjernet fra utvalget. Det er ',
    sum(aux$PainExperiencesNoActivity[aux$Aar > rap_aar-4] < 2), ' pasienter som
    har prescore 1 over hele perioden.') ),
  include.rownames=FALSE)
...
# aux$pstEndringSmerteAktiv <-
#   (aux$PainExperiencesActivity - aux$PainExperiencesActivity_post)/
#   aux$PainExperiencesActivity*100

aux$Indikator <- 0
# aux$Indikator[aux$pstEndringSmerteAktiv >= 30 ] <- 1
aux$Indikator[(aux$PainExperiencesActivity -
                 aux$PainExperiencesActivity_post) >= 2 ] <- 1
...
print(xtable::xtable(
  ...
  digits=c(0,0,0,0,0,1),
  caption=paste0(
    'Andel med 2 poeng eller mer reduksjon i smerte i aktivitet Individer med
    smertescore = 0 ved inklusjon er fjernet fra utvalget. Det er ',
    sum(aux$PainExperiencesActivity[aux$Aar > rap_aar-4] < 2), ' pasienter som
    har prescore 1 over hele perioden.')),
  include.rownames=FALSE)
@
...
% # aux$tilbake_i_arbeid[which((aux$WorkingPercent_post -
% #                               aux$WorkingPercent) >= 0 &
% #                              aux$WorkingStatus == 1)] <- 1
% #
% # tabell2 <- nnrrIndikatorTabell(aux |> dplyr::filter(Aar > rap_aar-4),
% #                                indikator = 'tilbake_i_arbeid')
% # print(xtable::xtable(
% #   tabell2,
% #   digits=c(0,0,0,0,0,1),
% #   caption='Andel delvis/fullt tilbake i arbeid. Utvalget består av alle
% #   som det finnes oppfølgingsdata på og er registrert som sykemeldt ved
% #   inklusjon. Hvis de i tillegg er registrert som i 100 \\% arbeid ved
% #   inklusjon fjernes de fra utvalget. For å regnes som tilbake i arbeid
% #   skal de ha registrert arbeidsstatus som Inntektsgivende arbeid, samt
% #   ha en prosentvis stilling ved oppfølging som er større eller lik den
% #   ved inklusjon.'),
% #   include.rownames=FALSE)
% # @
...
tabell1 <- nnrrIndikatorTabell(aux |> dplyr::filter(Aar > rap_aar-4),
                               indikator = 'NytteAvBehandling')
print(xtable::xtable(tabell1,
                     digits=c(0,0,0,0,0,1),
                     caption='Pasientrapportert nytte av behandling'),
      include.rownames=FALSE)
...
print(xtable::xtable(tabell1,
                     digits=c(0,0,0,0,0,1),
                     caption='Andel fornøyd med eller nøytral til behandlingen'),
      include.rownames=FALSE)
...
tabell1 <- nnrrIndikatorTabell(Oppf_data |> dplyr::filter(Aar > rap_aar-4),
                               indikator = 'regstatus_post')
print(xtable::xtable(tabell1,
                     digits=c(0,0,0,0,0,1),
                     caption='Andel oppfølgingsskjema utfylt'),
      include.rownames=FALSE)
...
<<'DG-tall NNRR', results='asis', echo=FALSE, message=FALSE, include=FALSE>>=

# dg_gml <- read.csv2("~/regdata/nnrr/dg_2023nnrr.csv") |>
#   dplyr::filter(ind_id == "nnrr_dg")
#
# dg_2024 <- tribble(
#   ~Sykehus, ~var, ~denominator, ~orgnr,
#   "Haukeland universitetssykehus", 741, 802, 974557746,
#   "Oslo Universitetssykehus", 1397, 1800, 974589095,
#   "Stavanger", 506, 632, 974703300,
#   "St. Olavs Hospital", 952, 1248, 974749025,
#   "Sørlandet sykehus", 767, 1026, 974733013,
#   "Sykehuset i Vestfold Rehabilitering Stavern", 966, 1395, 974575396,
#   "Vestre Viken Drammen", 264, 378, 974631326,
#   "UNN Tromsø", 440, 529, 974795787,
#   "Møre og Romsdal, Ålesund", 105, 119, 974747138,
#   "Helgelandssykehuset, Sandnessjøen", 58, 62, 974795477,
#   "UNN Harstad", 27, 43, 974795639,
#   "Nordlandssykehuset", 87, 110, 993562718,
#   "Sykehuset Innlandet", 102, 131, 874631752,
#   "Levanger", 61, 100, 974754118,
#   "Helse Fonna", 12, 15, 996328112
# ) |>
...
#
# dg_samlet <- dplyr::bind_rows(dg_gml, dg_2024)
...
write.csv2(
  Indikatorer,
  paste0("C:/Users/kth200/regdata/nnrr/behandlingskvalitet/beh_kval_",
         Sys.Date(), ".csv"),
  row.names = F)

dg <- openxlsx::read.xlsx(
  "C:/Users/kth200/regdata/nnrr/behandlingskvalitet/Dekningsgrad Helårstall 2025 endelig.xlsx") |>
  dplyr::mutate(
    RESH = stringr::str_trim(RESH) |> as.numeric(),
    orgnr = map_resh_orgnr$orgnr_sh[match(RESH, map_resh_orgnr$resh)],
    year = 2025,
    ind_id = "nnrr_dg",
    context = "caregiver") |>
  dplyr::filter(!is.na(RESH)) |>
  dplyr::rename(var = Registreringer,
                denominator = Innrapportert.aktuelle) |>
  dplyr::select(context, orgnr, year, var, denominator, ind_id)

write.csv2(
  dg,
  paste0("C:/Users/kth200/regdata/nnrr/behandlingskvalitet/dg_", Sys.Date(), ".csv"),
  row.names = F)

#
# dg <- openxlsx::read.xlsx(
#   "C:/regdata/nnrr/behandlingskvalitet/Dekningsgrad Halvårstall 2025.xlsx") |>
#   dplyr::mutate(
#     RESH = stringr::str_trim(RESH) |> as.numeric(),
#     orgnr = map_resh_orgnr$orgnr_sh[match(RESH, map_resh_orgnr$resh)],
#     year = 2025,
...
# write.csv2(
#   dg,
#   paste0("C:/regdata/nnrr/behandlingskvalitet/dg_", Sys.Date(), ".csv"),
#   row.names = F)
