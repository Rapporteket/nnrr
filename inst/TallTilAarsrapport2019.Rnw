...
\title{Figurer og tabeller til 2019 årsrapport for NNRR}
\author{NNRR}
...
Oppf_aktuell <- RegDataAll[RegDataAll$Besoksdato >= '2018-07-01' & RegDataAll$Besoksdato <= '2019-06-30', ]

tab_oppf <- Oppf_aktuell %>% group_by(SykehusNavn) %>%
  summarise('Antall konsultatsjoner' = sum(regstatus),
            'Antall oppfølginger' = sum(regstatus_post),
            'Oppfølgingsrate (\\%)' = sum(regstatus_post)/sum(regstatus)*100)

print(xtable::xtable(tab_oppf, digits=0, align=c('l', 'l', 'l', rep('r', ncol(tab_oppf)-2)), caption = 'Oversikt over oppfølgingsprosent per enhet for konsultasjoner i perioden 1.juli 2018 til 30. juni 2019'), include.rownames=FALSE, sanitize.text.function = identity)
...
tab5 <- filter(tab5,  Aar %in% c(2015, 2016, 2017, 2018, 2019))
...
nnrrFigGjsnGrVarTid(RegDataAll, valgtVar=valgtVar, datoTil='2019-12-31', outfile=outfile, tittel='Gjennomsnittsalder')
if (!is.na(annetformat)) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigGjsnGrVarTid(RegDataAll, valgtVar=valgtVar, datoTil='2019-12-31', outfile=outfile, tittel='Gjennomsnittsalder')
}
...
nnrrFigAndeler(RegDataAll, valgtVar = valgtVar, enhetsUtvalg = 0, reshID = 0, outfile=outfile,
               datoFra = '2019-01-01', datoTil = '2019-12-31')
if (!is.na(annetformat)) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigAndeler(RegDataAll, valgtVar = valgtVar, enhetsUtvalg = 0, reshID = 0, outfile=outfile,
               datoFra = '2019-01-01', datoTil = '2019-12-31')
...
tab9 <- RegDataAll %>% filter(Besoksdato>='2018-07-01' & Besoksdato<='2019-06-30' & regstatus_pre == 1 &
...
tab10 <- RegDataAll %>% filter(Besoksdato>='2018-07-01' & Besoksdato<='2019-06-30' & regstatus_pre == 1 &
...
tmp <- nnrrFigGjsnPrePostGrVar(RegData = RegDataAll, valgtVar = 'PainExperiencesNoActivity', outfile = outfile, datoFra = '2018-07-01', datoTil = '2019-06-30')
if (!is.na(annetformat)) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigGjsnPrePostGrVar(RegData = RegDataAll, valgtVar = 'PainExperiencesNoActivity', outfile = outfile, datoFra = '2018-07-01', datoTil = '2019-06-30')
}

outfile <- "Smerte_i_aktivitet.pdf"
tmp <- nnrrFigGjsnPrePostGrVar(RegData = RegDataAll, valgtVar = 'PainExperiencesActivity', outfile = outfile, datoFra = '2018-07-01', datoTil = '2019-06-30')
if (!is.na(annetformat)) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigGjsnPrePostGrVar(RegData = RegDataAll, valgtVar = 'PainExperiencesActivity', outfile = outfile, datoFra = '2018-07-01', datoTil = '2019-06-30')
}

outfile <- "ODI_PrePost.pdf"
tmp <- nnrrFigGjsnPrePostGrVar(RegData = RegDataAll, valgtVar = 'ODI_PrePost', outfile = outfile, datoFra = '2018-07-01', datoTil = '2019-06-30')
if (!is.na(annetformat)) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigGjsnPrePostGrVar(RegData = RegDataAll, valgtVar = 'ODI_PrePost', outfile = outfile, datoFra = '2018-07-01', datoTil = '2019-06-30')
}

outfile <- "NDI_PrePost.pdf"
tmp <- nnrrFigGjsnPrePostGrVar(RegData = RegDataAll, valgtVar = 'NDI_PrePost', outfile = outfile, datoFra = '2018-07-01', datoTil = '2019-06-30')
if (!is.na(annetformat)) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigGjsnPrePostGrVar(RegData = RegDataAll, valgtVar = 'NDI_PrePost', outfile = outfile, datoFra = '2018-07-01', datoTil = '2019-06-30')
}
@
...
Tabell_pasrapp_beh <- aux %>% group_by(Aar, SykehusNavn) %>%
  summarise('Ingen behandling' = sum(!TreatmentPostHospital & !TreatmentPostHospitalSurgery, na.rm = T),
            'Operasjon' = sum(TreatmentPostHospital, na.rm = T),
            'Individuelt behandlingstilbud' = sum(TreatmentPostIndividual),
...
# print(xtable::xtable(Tabell_pasrapp_beh, digits=c(rep(0, dim(Tabell_pasrapp_beh)[2]),1), caption='Pasientrapportert behandling på nakke - og ryggpoliklinikken.'), include.rownames=FALSE)
...
nnrrFigAndeler(RegDataAll, valgtVar = 'pasrapp_beh_klinikk', enhetsUtvalg = 0, reshID = 0,
               datoFra = '2019-01-01', datoTil = '2019-12-31', outfile = outfile)
if (!is.na(annetformat)) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigAndeler(RegDataAll, valgtVar = 'pasrapp_beh_klinikk', enhetsUtvalg = 0, reshID = 0,
               datoFra = '2019-01-01', datoTil = '2019-12-31', outfile = outfile)
...
lag_beh_tabell<- function(regdata, varnavn, etikett) {
  ...
  tabell <- regdata %>% group_by(Aar, SykehusNavn, Variabel) %>% summarise('Antall' = sum(indx))
  tmp <- regdata %>% group_by(Aar, Variabel) %>% summarise('Antall' = sum(indx))
  tmp$SykehusNavn <- 'Zmp'
  tmp <- tmp[, c(1,4,2,3)]
  ...
  tabell$Aar[-match(unique(tabell$Aar), tabell$Aar)] <- NA
  tabell$Var <- etikett
...
Indikator <- RegDataAll[RegDataAll$Aar <= 2019,]
