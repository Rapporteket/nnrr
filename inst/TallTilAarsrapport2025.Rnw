...
\title{Figurer og tabeller til 2025 årsrapport for NNRR}
\author{NNRR}
...
datasti <- "C:/Users/kth200/regdata/nnrr/aarsrapp2025/"
...
figfolder <- paste0(datasti, "figurer_aarsrapp/")
if (!dir.exists(figfolder)) {
  dir.create(figfolder)
}
tabfolder <- paste0(datasti, "tabeller_aarsrapp/")
...
tot <- RegData[, c("regstatus", "regstatus_pre", "regstatus_post", "Aar")] %>%
  group_by (Aar) %>%
  summarise('Pasientskjema' = sum(regstatus_pre),
            'Behandlerskjema' = sum(regstatus),
            'Oppfølgingsskjema' = sum(regstatus_post)) %>% ungroup()
...
print(xtable::xtable(
  tab0[,-5],
  digits=0, align=c('l', 'l', 'l', rep('r', ncol(tab0)-3)),
  caption = 'Oversikt over registreringer. Kun pasientskjema som har et
  tilhørende behandlerskjema er inkludert'),
...
tab_oppf_alle <- RegDataAll[RegDataAll$regstatus_pre == 1, ] %>%
  group_by(SykehusNavn) %>%
  summarise(
    "Ant.aktuell.6mnd" = sum(
      regstatus[Besoksdato >= paste0(rap_aar-1, "-07-01") &
                  Besoksdato <= paste0(rap_aar, "-06-30")]),
    "Ant.aktuell.12mnd" = sum(
      regstatus[Besoksdato >= paste0(rap_aar-1, "-01-01") &
                  Besoksdato <= paste0(rap_aar-1, "-12-31")]),
    "fulgtopp.6mnd" = sum(
      regstatus_post[
        Besoksdato >= paste0(rap_aar-1, "-07-01") &
          Besoksdato <= paste0(rap_aar, "-06-30")]),
    "fulgtopp.12mnd" = sum(
      regstatus_post2[
        Besoksdato >= paste0(rap_aar-1, "-01-01") &
          Besoksdato <= paste0(rap_aar-1, "-12-31")]),
    "fulgtopp.begge" = sum(
      regstatus_post[
        Besoksdato >= paste0(rap_aar-1, "-01-01") &
  ...
  mutate(Andel.6mnd = fulgtopp.6mnd/Ant.aktuell.6mnd*100,
         Andel.12mnd = fulgtopp.12mnd/Ant.aktuell.12mnd*100,
         Andel.begge = fulgtopp.begge/Ant.aktuell.12mnd*100) |>
  select(SykehusNavn, Ant.aktuell.6mnd, Ant.aktuell.12mnd,
         Andel.6mnd, Andel.12mnd, Andel.begge)
...
write.csv2(tab_oppf_alle, paste0(tabfolder, "tab_oppfolging.csv"),
...
tab7 <- RegData[, c(as.character(BehSpeshelsetj$varnavn),
                    "Treatment_GroupInterdisciplinary", "Aar",
                    "Treatment_TreatmentInSpecialistServices")] %>%
  group_by (Aar) %>%
  summarise(
    'Behandling i spesialhelsetjenesten' =
      sum(Treatment_TreatmentInSpecialistServices),
    'Henvist til vurdering av operasjon' =
      sum(Treatment_FollowUpOperation),
    'Anbefaler henvisning til annet opptrenings /rehabiliteringssenter' =
      sum(Treatment_TreatmentOtherRehabCentre),
    'Kontroll etter vurdering eller behandling' =
      sum(Treatment_ControlAfterReviewOrTreatment),
    'Individuell oppfølging 1-2 ganger' =
      sum(Treatment_IndividualFollowUp1to2Times),
    'Individuell tverrfaglig behandling 1-3 ganger' =
      sum(Treatment_InvidualInterdisciplinary==1),
    'Individuell tverrfaglig behandling 4-10 ganger' =
      sum(Treatment_InvidualInterdisciplinary==2),
    'Individuell tverrfaglig behandling mer enn 10 ganger' =
      sum(Treatment_InvidualInterdisciplinary==3),
    'Tverrfaglig behandling i gruppe 1-3 ganger' =
      sum(Treatment_GroupInterdisciplinary2018==1 | Treatment_GroupInterdisciplinary==1),
    'Tverrfaglig behandling i gruppe 4-10 ganger' =
      sum(Treatment_GroupInterdisciplinary2018 %in% 2:3 | Treatment_GroupInterdisciplinary==2),
    'Tverrfaglig behandling i gruppe mer enn 10 ganger' =
      sum(Treatment_GroupInterdisciplinary2018==4 | Treatment_GroupInterdisciplinary==3)
  ) %>%
...
tab7_v2 <- RegData |>
  dplyr::summarise(
    'Behandling i spesialhelsetjenesten' =
      sum(Treatment_TreatmentInSpecialistServices, na.rm = TRUE),
    'Henvist til vurdering av operasjon' =
      sum(Treatment_FollowUpOperation, na.rm = TRUE),
    'Henvisning til annet opptrenings/rehabiliteringssenter' =
      sum(Treatment_TreatmentOtherRehabCentre, na.rm = TRUE),
    'Individuell monofaglig oppfølging (1-3 ganger)' =
      sum(Treatment_IndividualMonoDiciplinary==1, na.rm = TRUE),
    'Individuell monofaglig oppfølging (4-10 ganger)' =
      sum(Treatment_IndividualMonoDiciplinary==2, na.rm = TRUE),
    'Individuell monofaglig oppfølging (>10 ganger)' =
      sum(Treatment_IndividualMonoDiciplinary==3, na.rm = TRUE),
    'Individuell tverrfaglig behandling (1-3 ganger)' =
      sum(Treatment_InvidualInterdisciplinary==1, na.rm = TRUE),
    'Individuell tverrfaglig behandling (4-10 ganger)' =
      sum(Treatment_InvidualInterdisciplinary==2, na.rm = TRUE),
    'Individuell tverrfaglig behandling (>10 ganger)' =
      sum(Treatment_InvidualInterdisciplinary==3, na.rm = TRUE),
    'Tverrfaglig behandling i gruppe (1-3 ganger)' =
      sum(Treatment_GroupInterdisciplinary2018==1, na.rm = TRUE),
    'Tverrfaglig behandling i gruppe (4-6 ganger)' =
      sum(Treatment_GroupInterdisciplinary2018==2, na.rm = TRUE),
    'Tverrfaglig behandling i gruppe (7-10 ganger)' =
      sum(Treatment_GroupInterdisciplinary2018==3, na.rm = TRUE),
    'Tverrfaglig behandling i gruppe (>10 ganger)' =
      sum(Treatment_GroupInterdisciplinary2018==4, na.rm = TRUE),
    .by = Aar
  ) |> mutate(across(c(2), as.integer)) |>
  tr_summarize_output()


print(xtable::xtable(tab7_v2, align=c('l', 'l', rep('r', ncol(tab7_v2)-1)),
...
\caption{Behandlingsløp utenfor spesialisthelsetjenesten}
\end{figure}
...
write.csv2(tab8, paste0(tabfolder, "tab8_nytte_av_behandling.csv"),
           row.names = T, fileEncoding = "Latin1")

print(xtable::xtable(
  tab8, digits=0, align=c('l',rep('R{0.4in}', ncol(tab8))),
  caption = ' Hvilken nytte/bedring mener du at du har hatt av behandlingen.
  Fordelt på sykehus.'),
  size = "\\scriptsize")
...
tab13 <- RegData[, c("SykehusNavn", "Treatment_NoTreatment", "Treatment_FollowUpMunicipality",
  ...
  summarise('Ingen beh.' = sum(Treatment_NoTreatment),
            ...
            'Fysio/manuell/psykomotorisk terapeut' = sum(Treatment_FollowUpFysioterapeut),
            ...
            'Arbeidsrettet oppfølging' = sum(Treatment_FollowUpMunicipalityOther),
            'Andre livsstilsrelaterte tilbud el. lign.' = sum(Treatment_FollowupOtherLifeStyle),
            N = n()) %>%
...
print(xtable::xtable(tab13, digits=0, align=c('l', 'l', rep('R{0.4in}', ncol(tab13)-1)),
                     caption = 'Behandlingsløype utenfor spesialisthelsetjenesten'),
      include.rownames=FALSE, size = "\\scriptsize")
...
tab15_v2[, -c(1, dim(tab15_v2)[2])] <- tab15_v2[, -c(1, dim(tab15_v2)[2])]/tab15_v2$N*100
tab15_v2[, -c(1, dim(tab15_v2)[2])] <- sapply(tab15_v2[, -c(1, dim(tab15_v2)[2])], function(x) {sprintf("%.1f%%", x)})
tab15_v2 <- tab15_v2[tab15_v2$N > 0, ] |> dplyr::arrange(SykehusNavn)


print(xtable::xtable(tab15_v2, digits=0, align=c('l', 'l', rep('R{0.35in}', ncol(tab15_v2)-1)),
                     caption = 'Behandlingsløype i spesialisthelsetjenesten'),
      include.rownames=FALSE, size = "\\scriptsize")

write.csv2(tab15_v2, paste0(tabfolder, "tab_behandling_spestjeneste.csv"),
           row.names = F, fileEncoding = "Latin1")
@
...
tab16 <- RegData |>
  filter(Treatment_TreatmentInSpecialistServices == 1) |>
  select("SykehusNavn", "Treatment_TrainingActivation",
         "Treatment_WorkFollowUp", "Treatment_CognitiveApproach",
         "Treatment_Education",
         "Treatment_AttentionAndBodyawareness_Training") %>%
  group_by (SykehusNavn) %>%
  summarise(
    'Trening / aktivisering' = sum(Treatment_TrainingActivation),
    ...
    'Oppmerksomhetstrening og kroppsbevissthet' =
      sum(Treatment_AttentionAndBodyawareness_Training),
    N = n()) %>%
  janitor::adorn_totals()
...
print(xtable::xtable(
  tab16, digits=0, align=c('l', 'l', rep('R{0.7in}', ncol(tab16)-1)),
...
print(xtable::xtable(
  tab17, digits=0, align=c('l',rep('R{0.6in}', ncol(tab17))),
  caption = ' Hvilken nytte/bedring mener du at du har hatt av behandlingen.
  Fordelt på sykehus.'),
  size = "\\small")
...
FigData <- nnrr::nnrrBeregnAndeler(
  RegDataAll, valgtVar=valgtVar, datoFra=paste0(rap_aar-1, "-07-01"),
...
RegDataAll$ingen_beh <- #!RegDataAll$TreatmentOperation &
  !RegDataAll$TreatmentPostIndividual &
  !RegDataAll$TreatmentPostGroupbased &
  !RegDataAll$TreatmentPostHospitalExtendedExamination
...
aux <- RegDataAll %>%
  ...
  summarise(
    "Ingen behandling" = paste0(sum(ingen_beh), " (",
                                round(100*sum(ingen_beh)/n(),1), "%)"),
    "Operasjon" = paste0(sum(TreatmentOther),
                         " (", round(100*sum(TreatmentOther)/n(),1), "%)"),
    "Individuell behandling" = paste0(
      sum(TreatmentPostIndividual),
      " (", round(100*sum(TreatmentPostIndividual)/n(),1), "%)"),
    "Gruppebehandling" = paste0(
      sum(TreatmentPostGroupbased),
      " (", round(100*sum(TreatmentPostGroupbased)/n(),1), "%)"),
    "Videre kontroller" = paste0(
      sum(TreatmentPostHospitalExtendedExamination), " (",
      round(100*sum(TreatmentPostHospitalExtendedExamination)/n(),1), "%)"),
    N=n())
...
print(xtable::xtable(
  aux, digits=0,
  align=c('l', 'l', rep('R{0.8in}', ncol(aux)-2), 'r'),
  caption='Pasientrapportert behandling på nakke - og ryggpoliklinikken.'),
  include.rownames=FALSE)

write.csv2(aux, paste0(tabfolder, "tab_pasientrapp_beh_klin.csv"),
           row.names = F, fileEncoding = "Latin1")
...
aux2 <- RegDataAll %>%
  ...
  summarise(
    'Trening hos fysioterapeut' =
      paste0(sum(TreatmentPhysioTherapist !=0), " (",
             round(100*sum(TreatmentPhysioTherapist !=0)/n(),1), "%)"),
    'Annen behandling hos fysioterapeut' =
      paste0(sum(TreatmentPhysioTherapistOther !=0), " (",
             round(100*sum(TreatmentPhysioTherapistOther !=0)/n(),1), "%)"),
    'Manuell terapi' = paste0(
      sum(TreatmentManuelTherapi !=0), " (",
      round(100*sum(TreatmentManuelTherapi !=0)/n(),1), "%)"),
    'Psykomotorisk fysioterapi'= paste0(
      sum(TreatmentPsychomotoricTherapy !=0), " (",
      round(100*sum(TreatmentPsychomotoricTherapy !=0)/n(),1), "%)"),
    'Kiropraktor'= paste0(sum(
      TreatmentChiropractor !=0), " (",
      round(100*sum(TreatmentChiropractor !=0)/n(),1), "%)"),
    'Annen behandling'= paste0(sum(
      TreatmentOtherTreatment !=0), " (",
      round(100*sum(TreatmentOtherTreatment !=0)/n(),1), "%)"),
    N=n())
...
samlet <- RegDataAll %>%
  filter(
    Besoksdato>=paste0(rap_aar-1, "-07-01") &
      ...
      regstatus_post == 1) %>%
  summarise(
    'Trening hos fysioterapeut' = paste0(sum(
      TreatmentPhysioTherapist !=0), " (",
      round(100*sum(TreatmentPhysioTherapist !=0)/n(),1), "%)"),
    'Annen behandling hos fysioterapeut' = paste0(sum(
      TreatmentPhysioTherapistOther !=0), " (",
      round(100*sum(TreatmentPhysioTherapistOther !=0)/n(),1), "%)"),
    'Manuell terapi' = paste0(sum(
      TreatmentManuelTherapi !=0), " (",
      round(100*sum(TreatmentManuelTherapi !=0)/n(),1), "%)"),
    'Psykomotorisk fysioterapi'= paste0(sum(
      TreatmentPsychomotoricTherapy !=0), " (",
      round(100*sum(TreatmentPsychomotoricTherapy !=0)/n(),1), "%)"),
    'Kiropraktor'= paste0(sum(
      TreatmentChiropractor !=0), " (",
      round(100*sum(TreatmentChiropractor !=0)/n(),1), "%)"),
...
print(xtable::xtable(
  aux2, digits=0,
  align=c('l', 'l', rep('R{0.8in}', ncol(aux2)-2), 'r'),
  caption='Antall pasientrapportert oppfølging utenfor sykehus ved 6 mnd oppfølging.'),
  include.rownames=FALSE,
  size = "\\scriptsize")
...
aux$ant_hoy_uhi <- rowSums(aux[
  , c("UhiQ1", "UhiQ2", "UhiQ3", "UhiQ4",
      "UhiQ8", "UhiQ9", "UhiQ10", "UhiQ11", "UhiQ12", "UhiQ13", "UhiQ14",
      "UhiQ15", "UhiQ16", "UhiQ17", "UhiQ18", "UhiQ19", "UhiQ20", "UhiQ21",
      "UhiQ22", "UhiQ23", "UhiQ24", "UhiQ25", "UhiQ26", "UhiQ27", "UhiQ28",
      "UhiQ29", "UhiQ30", "UhiQ31", "UhiQ32", "UhiQ33")], na.rm = T)
aux$ant_besvart_uhi <- rowSums(is.na(aux[
  , c("UhiQ1", "UhiQ2", "UhiQ3", "UhiQ4",
      "UhiQ8", "UhiQ9", "UhiQ10", "UhiQ11", "UhiQ12", "UhiQ13", "UhiQ14",
      "UhiQ15", "UhiQ16", "UhiQ17", "UhiQ18", "UhiQ19", "UhiQ20", "UhiQ21",
      "UhiQ22", "UhiQ23", "UhiQ24", "UhiQ25", "UhiQ26", "UhiQ27", "UhiQ28",
...
aux$Teller <- 0
aux$Teller[aux$TreatmentAbilityToWorkFunctionLevel %in% 1] <- 1
...
outfile <- paste0(figfolder, outfile)
nnrrFigIndikator(indikator,
                 c('Andel kartlagt funksjonsevne',
                   'relatert til arbeid og utdanning'),
                 outfile=outfile)
if (annetformat != "pdf") {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigIndikator(indikator,
