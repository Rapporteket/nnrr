...
pasientsvar_pre <- read.table('I:/nnrr/DataDump_Prod_1a%3aSpørreskjema+før+behandling_2018-08-03_red.csv', sep=';', header=T, stringsAsFactors = F)
legeskjema <- read.table('I:/nnrr/DataDump_Prod_1b%3aRegistreringsskjema+poliklinikk_2018-08-03.csv', sep=';', header=T, fileEncoding = 'UTF-8-BOM', stringsAsFactors = F)
pasientsvar_post <- read.table('I:/nnrr/DataDump_Prod_2%3aSpørreskjema+etter+behandling_2018-08-03.csv', sep=';', header=T, fileEncoding = 'UTF-8-BOM', stringsAsFactors = F)
...
# RegDataAll <- RegData
RegData <- RegData[which(RegData$Aar >= 2016 & RegData$Aar <= 2017), ]
...
print(xtable::xtable(tab0, digits=0, align=c('l', 'l', 'l', rep('r', ncol(tab0)-2))), include.rownames=FALSE, sanitize.text.function = identity)
@
