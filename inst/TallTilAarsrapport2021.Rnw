...
if (!is.na(annetformat)) {
  outfile <- paste0(substr(outfile, 1, nchar(outfile)-4), annetformat)
  nnrrFigGjsnPrePostGrVar_v2(RegData = RegDataAll, valgtVar = 'PainExperiencesNoActivity', outfile = outfile, datoFra = paste0(rap_aar-1, "-07-01"), datoTil = paste0(rap_aar, "-06-30"))
}
