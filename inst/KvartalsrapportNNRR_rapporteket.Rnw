...
if (!exists("RegData")) {RegData <- nnrr::nnrrHentRegData()}
...
figstr <- 0.61
figfolder <- paste0(normalizePath(tempdir(), winslash = "/"), "/")
...
datoFra_sistkvartal <- lubridate::floor_date(
  as.Date(datoTil),
  unit="month") %m-% months(2)

aux <- RegData[RegData$Besoksdato >= datoFra_sistkvartal &
                 RegData$Besoksdato <= datoTil, ]
...
HelseIArbeid <- table(aux[, c("ParticipantHelseIArbeid", "avd")])
HelseIArbeid[,1] <- round(HelseIArbeid[,1]/N[1]*100, 1)
HelseIArbeid[,2] <- round(HelseIArbeid[,2]/N[2]*100, 1)
HelseIArbeid[,1]  <- paste0(HelseIArbeid[,1] , " \\%")
HelseIArbeid[,2]  <- paste0(HelseIArbeid[,2] , " \\%")
samlet <- rbind(N, rep(NA, dim(kjonn)[2]),
                kjonn, rep(NA, dim(kjonn)[2]),
                alder, rep(NA, dim(kjonn)[2]),
                rep(NA, dim(kjonn)[2]),
                familiestatus,
                rep(NA, dim(kjonn)[2]),
                rep(NA, dim(kjonn)[2]),
                utdanning, rep(NA, dim(kjonn)[2]),
                rep(NA, dim(kjonn)[2]),
                HelseIArbeid)
rownames(samlet) <- 1:dim(samlet)[1]
samlet <- as.data.frame(samlet)
samlet$radnavn <- c('\\textbf{N}', '\\textbf{Kjønn}',
                    '\\quad Kvinner', '\\quad Menn', '',
                    '\\textbf{Alder, gj.snitt (SD)}', '',
                    '\\textbf{Sivilstatus}',
                    '\\quad Gift/Reg. partner',
                    '\\quad Samboende', '\\quad Enslig',
                    '\\quad Ikke svart', '',
                    '\\textbf{Utdanning}',
                    '\\quad Grunnskole (7-10 år)',
                    '\\quad Vgs. yrkesfaglig',
                    '\\quad Vgs. allmennfaglig',
                    '\\quad Høyskole/univ\\textless 4år',
                    '\\quad Høyskole/univ\\textgreater 4 år',
                    '\\quad Ikke svart', '',
                    '\\textbf{Helse i arbeid}',
                    '\\quad Nei',
                    '\\quad Ja'
                    )
samlet <- samlet[, 3:1]
...
print(xtable::xtable(
  samlet, align=c('l', 'l', rep('r', ncol(samlet)-1)),
  caption = "Basert på registreringer siste avsluttede kvartal"),
...
valgtVar <- "PatientAge"
plotdata <- nnrr::nnrrBeregnAndeler(RegData, valgtVar=valgtVar,
                                    datoFra=datoFra_sistkvartal,
...
plotdata <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar, datoFra=datoFra,
  datoTil=datoTil, enhetsUtvalg=1,
  minald=0, maxald=130, erMann=99,
  outfile=outfile, reshID=reshID,
...
plotdata <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar, datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=1,
...
plotdata1 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar,
...
plotdata2 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar,
  datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=1,
  minald=0, maxald=130, erMann=99,
  outfile=outfile, reshID=reshID,
...
outfile <- paste0(figfolder, "tilbake_i_arbeid.pdf")
nnrr::nnrrTidsplot_6og12mnd(plotdata1 = plotdata1,
                            plotdata2 = plotdata2,
                            outfile=outfile,
                            inkl_konf = 0)

valgtVar <- "mer_jobb_6mnd"
plotdata1 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar, datoFra=datoFra,
  datoTil=datoTil, enhetsUtvalg=1,
  minald=0, maxald=130, erMann=99, outfile=outfile, reshID=reshID,
  tidsenhet="Kvartal", datovar = "dato_oppfolg")
valgtVar <- "mer_jobb_12mnd"
plotdata2 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar,
  datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=1,
  minald=0, maxald=130, erMann=99, outfile=outfile, reshID=reshID,
...
outfile <- paste0(figfolder, "mer_arbeid.pdf")
nnrr::nnrrTidsplot_6og12mnd(plotdata1 = plotdata1,
                            plotdata2 = plotdata2,
                            outfile=outfile,
                            inkl_konf = 0)
...
outfile <- paste0(figfolder, "tilbake_i_arbeid_tverrfaglig.pdf")
nnrr::nnrrTidsplot_6og12mnd(plotdata1 = plotdata1,
                            plotdata2 = plotdata2,
                            outfile=outfile,
                            inkl_konf = 0)
...
plotdata1 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar, datoFra=datoFra,
  datoTil=datoTil, enhetsUtvalg=1, tverrfaglig = 1,
  minald=0, maxald=130, erMann=99, outfile=outfile, reshID=reshID,
  tidsenhet="Kvartal", datovar = "dato_oppfolg")
valgtVar <- "mer_jobb_12mnd"
plotdata2 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar,
  datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=1, tverrfaglig = 1,
  minald=0, maxald=130, erMann=99, outfile=outfile, reshID=reshID,
  tidsenhet="Kvartal", datovar = "dato_oppfolg2")

outfile <- paste0(figfolder, "mer_arbeid_tverrfaglig.pdf")
nnrr::nnrrTidsplot_6og12mnd(plotdata1 = plotdata1,
                            plotdata2 = plotdata2,
                            outfile=outfile,
                            inkl_konf = 0)
...
valgtVar <- "bedring_smerte_hvile_6mnd_v2"
plotdata1 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar,
  datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=1,
  minald=0, maxald=130, erMann=99, outfile=outfile, reshID=reshID,
  tidsenhet="Kvartal", datovar = "dato_oppfolg")
valgtVar <- "bedring_smerte_hvile_12mnd_v2"
plotdata2 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar,
  datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=1,
  minald=0, maxald=130, erMann=99, outfile=outfile, reshID=reshID,
...
valgtVar <- "bedring_smerte_aktiv_6mnd_v2"
plotdata1 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar,
  datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=1,
  minald=0, maxald=130, erMann=99, outfile=outfile, reshID=reshID,
  tidsenhet="Kvartal", datovar = "dato_oppfolg")
valgtVar <- "bedring_smerte_aktiv_12mnd_v2"
plotdata2 <- nnrr::nnrrBeregnAndelTid(
  RegData, valgtVar=valgtVar,
  datoFra=datoFra, datoTil=datoTil, enhetsUtvalg=1,
  minald=0, maxald=130, erMann=99, outfile=outfile, reshID=reshID,
...
nnrrFigAndelGrvar(
  RegData, valgtVar="opplevd_nytte_beh_6mnd", datoFra=datoFra,
  datoTil=datoTil, enhetsUtvalg=1, datovar="dato_oppfolg",
...
nnrrFigAndelGrvar(
  RegData, valgtVar="opplevd_nytte_beh_12mnd",
  datoFra=datoFra, datoTil=datoTil,
...
\caption{Andel pasienter som 6 og 12 mnd etter utredning/behandling har hatt klinisk bedring i funksjon (ODI) (10 poeng eller mer) i egen spesialisthelsetjeneste sammenlignet med landet for øvrig siste 4 kvartal.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}tilbake_i_arbeid.pdf}
\caption{Andel pasienter som 6 og 12 mnd etter utredning/behandling er tilbake i fullt arbeid.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}tilbake_i_arbeid_tverrfaglig.pdf}
\caption{Andel pasienter som 6 og 12 mnd etter utredning/behandling er tilbake i fullt arbeid og har mottatt tverrfaglig behandling}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}mer_arbeid.pdf}
\caption{Andel pasienter som 6 og 12 mnd etter utredning/behandling er tilbake i fullt arbeid eller har lavere grad sykemelding.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr}\textwidth]{\Sexpr{figfolder}mer_arbeid_tverrfaglig.pdf}
\caption{Andel pasienter som 6 og 12 mnd etter utredning/behandling er tilbake i fullt arbeid eller har lavere grad sykemelding og har mottatt tverrfaglig behandling}
\end{figure}
...
\caption{Andel pasienter som 6 og 12 mnd etter utredning/behandling har hatt klinisk bedring av smerte (2 poeng eller mer reduksjon) i hvile i egen spesialisthelsetjeneste sammenlignet med landet for øvrig siste 4 kvartal.}
\end{figure}

\begin{figure}[ht]
\centering
\includegraphics[width=\Sexpr{figstr*0.9}\textwidth]{\Sexpr{figfolder}bedring_smerte_aktiv.pdf}
\caption{Andel pasienter som 6 og 12 mnd etter utredning/behandling har hatt klinisk bedring av smerte (2 poeng eller mer reduksjon) i aktivitet i egen spesialisthelsetjeneste sammenlignet med landet for øvrig siste 4 kvartal.}
\end{figure}
