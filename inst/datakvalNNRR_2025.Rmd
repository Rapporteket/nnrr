...
# kodebok1a <- xlsx::read.xlsx("~/mydata/nnrr/kodebok_nnrr_april2024.xlsx", sheetIndex = 6, header = T)
# kodebok1b <- xlsx::read.xlsx("~/mydata/nnrr/kodebok_nnrr_april2024.xlsx", sheetIndex = 7, header = T)
# kodebok2 <- xlsx::read.xlsx("~/mydata/nnrr/kodebok_nnrr_april2024.xlsx", sheetIndex = 5, header = T)
# kodebok3 <- xlsx::read.xlsx("~/mydata/nnrr/kodebok_nnrr_april2024.xlsx", sheetIndex = 4, header = T)


kodebok1a <- readxl::read_xlsx("C:/Users/kth200/regdata/nnrr/metadata_NNRR_2026.05.12-PROD-SOX.xlsx", sheet = 6)
kodebok1b <- readxl::read_xlsx("C:/Users/kth200/regdata/nnrr/metadata_NNRR_2026.05.12-PROD-SOX.xlsx", sheet = 3)
kodebok2 <- readxl::read_xlsx("C:/Users/kth200/regdata/nnrr/metadata_NNRR_2026.05.12-PROD-SOX.xlsx", sheet = 9)
kodebok3 <- readxl::read_xlsx("C:/Users/kth200/regdata/nnrr/metadata_NNRR_2026.05.12-PROD-SOX.xlsx", sheet = 18)
...
knitr::kable(Oppsum, 
             caption = 'Antallet felter på NNRR sine skjema etter felttype')
```

Denne rapporten fokuserer på tre datatyper, Bool, Enum og Numerisk siden disse utgjør festeparten av feltene. Her følger en kort forklaring til datatypene.
...
skjema1b <- rapbase::loadRegData(
  registryName,
  "SELECT * FROM behandlerskjema_1")
skjema1a <- rapbase::loadRegData(
  registryName,
  "SELECT * FROM pasientskjema_foer_behand_2")
skjema2 <- rapbase::loadRegData(
  registryName,
  "SELECT * FROM pasientskjema_6_maaneder__3")
skjema3 <- rapbase::loadRegData(
  registryName,
  "SELECT * FROM pasientskjema_12_maaneder_8")

dato_var1a <- as.character(kodebok1a$Variabelnavn)[which(as.character(kodebok1a$Felttype) == 'DateTime')]
dato_var1a <- intersect(dato_var1a, names(skjema1a))
dato_var1b <- as.character(kodebok1b$Variabelnavn)[which(as.character(kodebok1b$Felttype) == 'DateTime')]
dato_var1b <- intersect(dato_var1b, names(skjema1b))
dato_var2 <- as.character(kodebok2$Variabelnavn)[which(as.character(kodebok2$Felttype) == 'DateTime')]
dato_var2 <- intersect(dato_var2, names(skjema2))
dato_var3 <- as.character(kodebok2$Variabelnavn)[which(as.character(kodebok3$Felttype) == 'DateTime')]
dato_var3 <- intersect(dato_var3, names(skjema3))
...
skjema3[, dato_var3] <- mutate_all(skjema3[, dato_var3], funs(as.Date(., format="%d.%m.%Y")))

skjema1b$Hoveddato <- skjema1b$S1b_DateOfCompletion
OppfAktuelle <- skjema1b |> 
  filter(Hoveddato >= paste0(rap_aar-1, '-01-01'),
         Hoveddato <= paste0(rap_aar, '-06-30')) |> 
  select(SkjemaGUID, Hoveddato)
...
skjema3$Hoveddato <- skjema3$DateOfCompletion
skjema3 <- skjema3[which(skjema3$Hoveddato >= datoFra & skjema3$Hoveddato <= datoTil), ]


boolske_var1a <- as.character(kodebok1a$Variabelnavn)[which(as.character(kodebok1a$Felttype) == 'Bool')]
enum_var1a <- as.character(kodebok1a$Variabelnavn)[which(as.character(kodebok1a$Felttype) == 'Enum')]
numerisk_var1a <- as.character(kodebok1a$Variabelnavn)[which(as.character(kodebok1a$Felttype) %in% c('Number'))]
boolske_var1b <- as.character(kodebok1b$Variabelnavn)[which(as.character(kodebok1b$Felttype) == 'Bool')]
boolske_var1b <- intersect(names(skjema1b), boolske_var1b)
enum_var1b <- as.character(kodebok1b$Variabelnavn)[which(as.character(kodebok1b$Felttype) == 'Enum')]
numerisk_var1b <- as.character(kodebok1b$Variabelnavn)[which(as.character(kodebok1b$Felttype) %in% c('Number'))]
boolske_var2 <- as.character(kodebok2$Variabelnavn)[which(as.character(kodebok2$Felttype) == 'Bool')]
boolske_var2 <- intersect(boolske_var2, names(skjema2))
enum_var2 <- as.character(kodebok2$Variabelnavn)[which(as.character(kodebok2$Felttype) == 'Enum')]
enum_var2 <- intersect(enum_var2, names(skjema2))
numerisk_var2 <- as.character(kodebok2$Variabelnavn)[which(as.character(kodebok2$Felttype) %in% c('Number'))]
numerisk_var2 <- intersect(numerisk_var2, names(skjema2))
boolske_var3 <- as.character(kodebok3$Variabelnavn)[which(as.character(kodebok3$Felttype) == 'Bool')]
boolske_var3 <- intersect(boolske_var3, names(skjema3))
enum_var3 <- as.character(kodebok3$Variabelnavn)[which(as.character(kodebok3$Felttype) == 'Enum')]
enum_var3 <- intersect(enum_var3, names(skjema3))
numerisk_var3 <- as.character(kodebok3$Variabelnavn)[which(as.character(kodebok3$Felttype) %in% c('Number'))]
numerisk_var3 <- c(intersect(numerisk_var3, names(skjema3)), 
                   "OdiScore", "NdiScore")
...
skjema3[, boolske_var3] <- apply(skjema3[, boolske_var3], 2, as.logical)
# skjema3[, boolske_var3] <- as.logical(skjema3[, boolske_var3])
...
enum_var3 <- enum_var3[sapply((skjema3[, enum_var3]), class) != "character"]

skjema1a[, which(substr(names(skjema1a), 1, 4) == "UhiQ")] <-
  skjema1a[, which(substr(names(skjema1a), 1, 4) == "UhiQ")] + 1

skjema1a[, enum_var1a] <- apply(skjema1a[, enum_var1a], 2, function(x){as.numeric(!(x == 0 | is.na(x)))})
skjema1b[, enum_var1b] <- apply(skjema1b[, enum_var1b], 2, function(x){as.numeric(!(x == 0 | is.na(x)))})
skjema2[, enum_var2] <- apply(skjema2[, enum_var2], 2, function(x){as.numeric(!(x == 0 | is.na(x)))})
skjema3[, enum_var3] <- apply(skjema3[, enum_var3], 2, function(x){as.numeric(!(x == 0 | is.na(x)))})
...
ingen_avkrysninger3 <- which(rowSums(skjema3[, boolske_var3])==0)
ingen_enum3 <- which(rowSums(skjema3[, enum_var3])<=1)
...
numerisk_var2 <- intersect(numerisk_var2, names(skjema2))
numerisk_var3 <- intersect(numerisk_var3, names(skjema3))
...
skjema3[, numerisk_var3] <- apply(skjema3[, numerisk_var3], 2, function(x){as.numeric(sapply(as.character(x), gsub, pattern = ",", replacement= "."))})
...
dobbel_1a <- names(sort(table(
  skjema1a$HovedskjemaGUID), 
  decreasing = T)[sort(table(skjema1a$HovedskjemaGUID), decreasing = T)>1])
dobbel_2 <- names(sort(table(
  skjema2$HovedskjemaGUID), 
  decreasing = T)[sort(table(skjema2$HovedskjemaGUID), decreasing = T)>1])
dobbel_1b <- names(sort(table(
  skjema1b$SkjemaGUID), 
  decreasing = T)[sort(table(skjema1b$SkjemaGUID), decreasing = T)>1])
dobbel_3 <- names(sort(table(
  skjema3$HovedskjemaGUID), 
  decreasing = T)[sort(table(skjema2$HovedskjemaGUID), decreasing = T)>1])
...
potensiellOppf6mnd <- OppfAktuelle |> 
  filter(Hoveddato >= paste0(rap_aar-1, '-06-30'))
potensiellOppf12mnd <- OppfAktuelle |> 
  filter(Hoveddato < paste0(rap_aar, '-01-01'))
manglerOppf6md <- length(setdiff(
  potensiellOppf6mnd$SkjemaGUID, skjema2$HovedskjemaGUID))
manglerOppf12md <- length(setdiff(
  potensiellOppf12mnd$SkjemaGUID, skjema3$HovedskjemaGUID))
...
Det er totalt `r dim(skjema1a)[1]` skjema 1a i utvalget og `r dim(skjema1b)[1]` skjema 1b. A disse er det `r mangler1b` pasientskjema som mangler klinikerskjema og `r mangler1a` klinikerskjema som mangler pasientskjema. Av de `r dim(potensiellOppf6mnd)[1]` som er aktuelle for 6mnd-oppfølging i `r rap_aar` det `r manglerOppf6md` som ikke har oppfølgingsdata. Av de `r dim(potensiellOppf12mnd)[1]` som er aktuelle for 12mnd-oppfølging i `r rap_aar` det `r manglerOppf12md` som ikke har oppfølgingsdata.
...
#### Skjema 3
```{r, echo = FALSE, warning=FALSE}
Andel_tom_num <- data.frame(
  Antall_tom=apply(skjema3[, numerisk_var3], 2, 
                   function(x){sum(is.na(x))}), N=dim(skjema3)[1])
Andel_tom_num$Andel <- round(
  Andel_tom_num$Antall_tom/Andel_tom_num$N*100, 1)
Andel_tom_num[,-2]
# tmp <- skjema1a[, c(numerisk_var1a, "SykehusNavn")] %>%
#   group_by(SykehusNavn) %>%
#   summarise_all(funs(sum(is.na(.)))) %>% tr_summarize_output()
...
#### Skjema 3
```{r, echo = FALSE, warning=FALSE}
Andel_tom_kat <- data.frame(Antall_tom=apply(skjema3[, enum_var3], 2, function(x){sum(x==0)}), N=dim(skjema3)[1])
Andel_tom_kat$Andel <- round(Andel_tom_kat$Antall_tom/Andel_tom_kat$N*100, 1)
Andel_tom_kat[,-2]
...
#### Skjema 3
```{r, echo = FALSE, warning=FALSE}
Andel_tom_boolsk <- data.frame(Antall_FALSE=apply(skjema3[, boolske_var3], 2, function(x){sum(!x)}), N=dim(skjema3)[1])
Andel_tom_boolsk$Andel <- round(Andel_tom_boolsk$Antall_FALSE/Andel_tom_boolsk$N*100, 1)
Andel_tom_boolsk[,-2]
```
...
Antall avkrysninger per skjema
```{r, echo = FALSE, warning=FALSE}
table(rowSums(skjema3[, boolske_var3]))
