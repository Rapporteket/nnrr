...
nnrrFigAndeler <- function(RegData,
  ...
  if (valgtVar %in% c(
    "AarsakSmerte_PasRap", "beh_kommunalt", "beh_spesialist",
  ...
  if (NHoved < 5 | (NRest < 5 & enhetsUtvalg == 1)) {
    ...
    vmarg <- switch(retn,
      V = 0,
      H = max(0, strwidth(grtxtpst, units = "figure", cex = cexgr) * 0.8)
    )
    ...
    if (retn == "V") {
      ...
      pos <- barplot(as.numeric(Andeler$Hoved),
        beside = TRUE, las = 1, ylab = ylabel, # main=tittel,
    ...
    if (retn == "H") {
      ...
      pos <- barplot(rev(as.numeric(Andeler$Hoved)),
        horiz = TRUE, beside = TRUE, las = 1, xlab = xlabel, # main=tittel,
