    ...
    function(input, output, session) {
      ...
      output$Tabell1 <- function() {
        ...
        if (input$enhetsUtvalg == 1) {
          ...
          names(Tabell_tid) <- c(
            "Tidsperiode", "Antall", "N", "Andel (%)",
            "KI_nedre", "KI_ovre", "Antall", "N", "Andel (%)",
            "KI_nedre", "KI_ovre"
          )
          Tabell_tid %>%
            knitr::kable("html", digits = c(0, 0, 0, 1, 1, 1, 0, 0, 1, 1, 1)) %>%
