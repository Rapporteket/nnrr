...
nnrrUtvalg <- function(RegData,
  ...
  utvalgTxt <- c(
    ...
    if ((minald > 0) | (maxald < 120)) {
      paste0(
        "Pasienter fra ", min(RegData$PatientAge, na.rm = T), " til ",
    ...
    if (tverrfaglig %in% 0:1) {
      paste0(
        "Tverrfaglig behandlet: ",
        c("Nei", "Ja")[tverrfaglig + 1]
      )
    },
    if (minHSCL > 1 | maxHSCL < 4) {
      paste0(
        "HSCL fra ", minHSCL,
        " til ", maxHSCL
      )
