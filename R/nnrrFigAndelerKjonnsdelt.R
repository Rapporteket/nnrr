...
nnrrFigAntallKjonnsdelt <- function(RegData, valgtVar, datoFra = "2014-01-01", datoTil = "2050-12-31", enhetsUtvalg = 1,
  ...
  if (enhetsUtvalg == 2) {
    RegData <- RegData[which(RegData$ReshId == reshID), ]
  }
  ...
  grtxt <- PlotParams$grtxt # grtxt2 <- PlotParams$grtxt2;
  ...
  if (min(NHoved) < 5 | (min(Nrest) < 5 & enhetsUtvalg == 1)) {
    ...
    pos <- barplot(AntHoved,
      beside = TRUE, las = 1, ylab = "Antall pasienter",
    ...
    legend("topright", c(paste0("Menn, N=", NHoved[1]), paste0("Kvinner, N=", NHoved[2])),
      bty = "n",
