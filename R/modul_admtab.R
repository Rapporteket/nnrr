  ...
  ui <- fluidPage(
    sidebarLayout(
      sidebarPanel(
        ...
        id = ns("id_adm_panel"),
        conditionalPanel(
          condition = sprintf("input['%s'] == 'id_ant_skjema'",
                              ns("admtabeller")),
          dateRangeInput(
        ...
        conditionalPanel(
          condition = sprintf("input['%s'] == 'id_ant_tid'",
                              ns("admtabeller")),
          # uiOutput(ns("sidebar_controls"))
          selectInput(
    ...
    function(input, output, session) {
      ...
      andre_adm_tab <- function() {
        if (input$adm_tidsenhet == 1) {
          ...
          fraDato <- lubridate::`%m-%`(tilDato,
                                       months(as.numeric(input$ant_mnd))) %>%
            lubridate::floor_date(unit="months")

          ant_skjema <- RegData |> dplyr::mutate(
            tid = factor(format(
              S1b_DateOfCompletion, format='%b-%y'),
        ...
        sketch <- htmltools::withTags(table(
          DT::tableHeader(ant_skjema[-dim(ant_skjema)[1], ]),
          DT::tableFooter(
            c('Sum' , as.numeric(ant_skjema[dim(ant_skjema)[1],
                                            2:dim(ant_skjema)[2]])))))
        list(ant_skjema=ant_skjema, sketch=sketch)
