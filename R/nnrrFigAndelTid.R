...
nnrrFigAndelTid <- function(RegData,
  ...
  Tidtxt <- switch(tidsenhet,
    Mnd = paste(substr(RegData$Aar[match(1:max(RegData$TidsEnhet), RegData$TidsEnhet)], 3, 4),
      sprintf("%02.0f", RegData$Mnd[match(1:max(RegData$TidsEnhet), RegData$TidsEnhet)]),
      sep = "."
    ),
    Kvartal = paste(substr(RegData$Aar[match(1:max(RegData$TidsEnhet), RegData$TidsEnhet)], 3, 4),
      sprintf("%01.0f", RegData$Kvartal[match(1:max(RegData$TidsEnhet), RegData$TidsEnhet)]),
      sep = "-"
    ),
    Halvaar = paste(substr(RegData$Aar[match(1:max(RegData$TidsEnhet), RegData$TidsEnhet)], 3, 4),
      sprintf("%01.0f", RegData$Halvaar[match(1:max(RegData$TidsEnhet), RegData$TidsEnhet)]),
      sep = "-"
    ),
  ...
  } # {indHovedUt <- which(RegData$UnitId != reshID)}
  ...
  if (length(indHoved) < 10 | (medSml == 1 & length(indRest) < 10)) {
    ...
    if (inkl_konf == 1) {
      ...
      plot(xskala, AndelHoved,
        xlim = c(xmin, xmax), ylim = c(ymin, ymax), type = "n", frame.plot = FALSE,
        ylab = c(paste0("Andel ", VarTxt), "inkl. 95% konfidensintervall"),
        xlab = switch(tidsenhet,
          Aar = "Intervensjonsår",
          Mnd = "Intervensjonsår og -måned",
          Kvartal = "Intervensjonsår og -kvartal",
          Halvaar = "Intervensjonsår og -halvår"
        ),
        xaxt = "n",
        sub = "(Tall i boksene angir antall intervensjoner)", cex.sub = cexgr
      ) # , axes=F)
      ...
      arrows(
        x0 = xskala, y0 = AndelHoved + h, x1 = xskala, y1 = replace(Konf[2, ], ind2, AndelHoved[ind2] + h),
      ...
      plot(AndelHoved,
        font.main = 1, type = "o", pch = "'", col = fargeHoved, xaxt = "n",
      ...
      if ((ymax > 10) & (ymax < 40)) {
        lines(range(xskala), rep(10, 2), col = farger[4])
      }
      if (ymax > 20) {
        lines(range(xskala), rep(20, 2), col = farger[4])
      }
      if ((ymax > 30) & (ymax < 40)) {
        lines(range(xskala), rep(30, 2), col = farger[4])
      }
      if (ymax > 40) {
        lines(range(xskala), rep(40, 2), col = farger[4])
      }
      if (ymax > 60) {
        lines(range(xskala), rep(60, 2), col = farger[4])
      }
      if (ymax > 80) {
        lines(range(xskala), rep(80, 2), col = farger[4])
      }
      if (ymax > 100) {
        lines(range(xskala), rep(100, 2), col = farger[4])
      }
    ...
    utData <- list(
      tittel = tittel, utvalgTxt = utvalgTxt, Andeler = list(AndelHoved = AndelHoved, AndelRest = AndelRest), Tidtxt = Tidtxt,
