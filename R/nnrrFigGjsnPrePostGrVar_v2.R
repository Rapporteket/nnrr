...
nnrrFigGjsnPrePostGrVar_v2 <- function(
  ...
  if (sammenlign == 0) {
    pos <- barplot(PlotMatrise,
      horiz = TRUE, main = tittel, las = 1,
    ...
    pos <- barplot(PlotMatrise[dim(PlotMatrise)[1]:1, ],
      beside = TRUE, horiz = TRUE, main = tittel, las = 1,
      col = farger[1:(sammenlign + 1)], border = "white", font.main = 1, xlim = c(0, xmax), ylim = c(0, ymax),
      names.arg = grtxt, cex.names = cexgr, xlab = "Gjsn.score"
    )
  }
