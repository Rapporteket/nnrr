...
nnrrPreprosess <- function(RegData) {
  ...
  boolske_var2 <- c(
    boolske_var2,
    paste0(
      as.character(nnrr::varnavn_2$Variabelnavn)[
        which(as.character(nnrr::varnavn_2$Felttype) == "Avkrysning")
      ],
      "_post"
    )
  )
  boolske_var <- intersect(
    c(boolske_var1b, boolske_var1a, boolske_var2),
    names(RegData)
  )
  RegData[, boolske_var] <-
  ...
  RegData[, c(
    "FormDate", "FormDate_pre", "FormDate_post",
  ...
  RegData$EducationLevel <- factor(RegData$EducationLevel,
    levels = c(1:5, 99),
    labels = c(
      "Grunnskole 7-10 år, framhaldskole eller folkehøyskole",
  ...
  RegData$PainDurationNow <- factor(
    RegData$PainDurationNow,
    levels = c(1:5, 99),
    labels = c(
      "Ingen smerter", "Mindre enn 3 måneder",
      "3 til 12 måneder",
      "1-2 år", "Mer enn 2", "Ikke svart"
    )
  )
  RegData$VentetidFraHenvisningTilTilbud_kat <-
    cut(RegData$VentetidFraHenvisningTilTilbud,
      breaks = c(0, 30, 60, 90, 180, 365, 100000),
      labels =
      )

  # RegData$NeckSurgery[RegData$NeckSurgery==0] <- 99
  RegData$NeckSurgery <- factor(RegData$NeckSurgery,
    levels = c(1:3),
    labels = c("Ja", "Nei", "Ukjent")
  )

  # RegData$BackSurgery[RegData$BackSurgery==0] <- 99
  RegData$BackSurgery <- factor(RegData$BackSurgery,
    levels = c(1:3),
    labels = c("Ja", "Nei", "Ukjent")
  )
  ...
  RegData$UseOfTreatmentLabel <- factor(RegData$UseOfTreatment,
    levels = nnrr::varnavn_2$kode[c(indekser_kodebok[-1], indekser_kodebok[1])],
  ...
  RegData$PhysicalActivityLabel <- factor(RegData$PhysicalActivity,
    levels = nnrr::varnavn_1a$kode[c(indekser_kodebok[-1], indekser_kodebok[1])],
    labels = nnrr::varnavn_1a$label[c(indekser_kodebok[-1], indekser_kodebok[1])]
  )

  RegData$WorkingStatus_label <- factor(RegData$WorkingStatus, levels = 0:8, labels = c(
    "Ikke utfylt", "Inntektsgivende arbeid",
  ...
  RegData$Tverrfaglig_vurdering[
    RegData$Tverrfaglig_vurdering_antall >= 2
  ] <- 1
  ...
  RegData$beh_spes_v2 <- factor(
    ...
    levels = 0:5,
    labels = c(
      "Ingen", "Individuell", "Gruppe, 1-3 ganger",
