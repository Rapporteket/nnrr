...
nnrrFigAndelerPrePost <- function(RegData = 0, valgtVar, datoFra = "2000-01-01", datoTil = "2050-01-01", reshID,
  ...
  if (NHoved < 1 | (enhetsUtvalg == 1 & Nrest < 1)) {
    ...
    vmarg <- switch(retn,
      V = 0,
      H = max(0, strwidth(grtxt, units = "figure", cex = cexgr) * 0.7)
    )
    ...
    if (retn == "V") {
      ...
      pos <- barplot(t(AndelerPP$Hoved),
        beside = TRUE, las = 1, ylab = "Andel pasienter (%)",
      ...
      if (enhetsUtvalg == 1) {
        ...
        legend("top", c(
          paste(c("Før, N=", "Etter, N="), NHoved, sep = ""),
        ...
        legend("top", c("Før", "Etter", paste("N=", NHoved, sep = "")),
          bty = "n",
    ...
    if (retn == "H") {
      ...
      pos <- barplot(t(AndelerPP$Hoved)[2:1, antGr:1],
        beside = TRUE, horiz = TRUE, main = "", las = 1,
      ...
      if (enhetsUtvalg == 1) {
        ...
        text.width = c(0.2, 0.2, 0.21) * xmax,
        bty = "n", pch = c(15, 15, 18), pt.cex = cexpt, # lty=c(NA,NA,NA),
        col = farger[c(2, 1, 3)], border = farger[c(2, 1, 3)], ncol = 3, cex = cexleg
        )
      } else {
